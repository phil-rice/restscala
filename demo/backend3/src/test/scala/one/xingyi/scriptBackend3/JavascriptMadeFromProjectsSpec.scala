...
class JavascriptMadeFromProjectsSpec extends UtilsSpec {
  ...
  it should "be in order, and express the lens" in {
    ...
    lensLines.map(implicitly[LensLineToJavascript]) shouldBe List(
        ...
        """function lens_person_line1_string(){return compose(lens("addresses"),compose(lensForItemInList(0),lens("line1")))};""",
        """function lens_person_line2_string(){return compose(lens("addresses"),compose(lensForLastItemInList(),lens("line2")))};""",
