...
class LensLanguageTest extends UtilsSpec {
  ...
  it should "parser simple items" in {
    ...
    lensParser("a,!address") shouldBe List(ChildLensDefn("a"), ViewLensDefn("address"))
    lensParser("a,*,!address") shouldBe List(ChildLensDefn("a"), new ListLensDefn(), ViewLensDefn("address"))
    lensParser("a,*,#0,!address") shouldBe List(ChildLensDefn("a"), new ListLensDefn(), ItemInListDefn(0), ViewLensDefn("address"))
    lensParser("a,*,#1,!address") shouldBe List(ChildLensDefn("a"), new ListLensDefn(), ItemInListDefn(1), ViewLensDefn("address"))
    lensParser("a,*,#last") shouldBe List(ChildLensDefn("a"), new ListLensDefn(), new LastItemInListDefn())
    lensParser("a,*,#last,!address") shouldBe List(ChildLensDefn("a"), new ListLensDefn(), new LastItemInListDefn(), ViewLensDefn("address"))
